...
Web画面はライブ`score.db`を読み取り専用で確認し、`Asia/Tokyo`の当日差分から10万までの
進捗を表示します。初回行など日次差分を安全に求められない場合だけ累積値へフォールバックします。
...
受信した累積打鍵数、`Asia/Tokyo`基準の本日打鍵数、観測日を`progress.json`へ原子的に
保存します。`runtime/`は`.gitignore`対象です。
...
送信ツールはDBを読み取り専用で5秒ごとに確認し、累積値または本日値が変化した時と30秒ごとの
heartbeatで送信します。Cockpitは日付切替後、新しい送信前でも前日状態の本日値を0へ戻します。
同じ累積値を再送しても二重加算されず、古い観測値やカウンター巻き戻りはMac側で拒否されます。
90秒受信がなければTOPページを`STALE`表示にします。WindowsとMacの
